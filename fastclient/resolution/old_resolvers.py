...
def _get_alias(parameter: param.Parameter, /) -> str:
    if not isinstance(parameter.default, BaseSingleParameter):
...
class ResolutionParameterManager(ParameterManager[Resolver]):
    ...
    def get_param(self, parameter: param.Parameter, /) -> BaseParameter:
        param: Optional[BaseParameter] = super().get_param(parameter)
    ...
    def resolve_all(
        ...
        for resolvable in resolvables:
            ...
            if not isinstance(parameter.default, BaseParameter):
                raise Exception("Cannot resolve non-param")
